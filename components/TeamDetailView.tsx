        ...
        <div className="animate-fade-in space-y-8">
            ...
            {activeTab === 'SCHEDULE' ? (
                <div className="space-y-6">
                    ...
                    {displayedGames.length === 0 ? <div className="p-16 text-center text-slate-400 dark:text-slate-500 border border-dashed border-slate-300 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20"><p>No {scheduleFilter.toLowerCase()} games found for {selectedSeasonLabel}.</p></div> : <div className="space-y-4">{displayedGames.map(game => { const isSelected = selectedGameId === game.id; const displayGame = scheduleFilter === 'PAST' ? { ...game, status: 'finished' as const } : game; return (<div key={game.id} className="transition-all duration-300"><GameCard game={displayGame} onSelect={onGameSelect} isSelected={isSelected} onTeamClick={onTeamClick} isFollowed={isGameFollowed ? isGameFollowed(displayGame as Game) : false} onToggleFollow={onToggleFollowGame} />{isSelected && <div className="relative mt-4 pl-4 sm:pl-6 sm:ml-4 border-l-2 border-slate-200 dark:border-slate-800 animate-fade-in"><div className="absolute -left-[9px] -top-4 w-4 h-8 rounded-bl-xl border-l-2 border-b-0 border-slate-200 dark:border-slate-800 bg-transparent opacity-0"></div>{isPredicting ? <div className="bg-white dark:bg-slate-900/60 rounded-3xl p-12 border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center text-center shadow-xl"><Loader2 size={48} className="text-slate-500 animate-spin mb-6" /><h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 font-display">Loading Data</h3><p className="text-slate-500 dark:text-slate-400 max-w-xs">Retrieving game details...</p></div> : (displayGame.status === 'finished' || scheduleFilter === 'PAST') ? <LiveGameView game={displayGame} gameDetails={gameDetails || null} prediction={null} isDarkMode={!!isDarkMode} onGenerateAnalysis={onGenerateAnalysis} onTeamClick={onTeamClick} /> : prediction ? (game.status === 'scheduled' ? <PredictionView game={game} prediction={prediction} isDarkMode={!!isDarkMode} onGenerateAnalysis={onGenerateAnalysis} gameDetails={gameDetails || null} onTeamClick={onTeamClick} /> : <LiveGameView game={game} gameDetails={gameDetails || null} prediction={prediction} isDarkMode={!!isDarkMode} onGenerateAnalysis={onGenerateAnalysis} onTeamClick={onTeamClick} />) : <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 text-center"><p>Unable to load data.</p></div>}</div>}</div>); })}</div>}
                </div>
            ) : activeTab === 'STATS' ? (
