    ...
    return (
        <div className="bg-white dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm flex flex-col transition-all duration-300 w-full h-full min-h-[220px]">
             ...
             {sortedPlayers.length === 0 ? (
                 <div className="p-8 text-center flex-1 flex items-center justify-center"><p className="text-xs font-medium text-slate-600 dark:text-slate-300 uppercase tracking-wide">Lineup Not Available</p></div>
             ) : (
                         ...
                         <div key={player.id} onClick={() => onPlayerClick && onPlayerClick(player.id)} className="flex items-center justify-between px-5 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors group">
                             <div className="flex items-center gap-4">
                                 ...
                                 <div className="flex flex-col">
                                     ...
                                     <span className="text-[9px] text-slate-600 dark:text-slate-300">{player.position} {player.jersey ? `• #${player.jersey}` : ''}</span>
                                 </div>
                             </div>
                             <div className="p-1.5 rounded-full text-slate-500 dark:text-slate-300 group-hover:bg-slate-100 dark:group-hover:bg-slate-800 group-hover:text-slate-700 dark:group-hover:text-slate-100 transition-all">
                                 <BarChart2 size={14} />
                             </div>
                         </div>
                     ))}
